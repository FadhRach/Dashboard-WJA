# feb/15/2023 04:56:50 by RouterOS 6.48
...
/queue simple
add disabled=yes dst=ether1-TO_FORTI max-limit=100M/100M name=VLAN40 target=\
    172.16.30.48/28
add disabled=yes dst=ether1-TO_FORTI max-limit=6M/6M name=VLAN50 target=\
    172.16.30.64/28
add disabled=yes dst=ether1-TO_FORTI max-limit=15M/15M name=VLAN60 target=\
    172.16.30.80/28
...
add comment="benns, faistby" distance=1 dst-address=172.16.50.78/32 gateway=\
    172.16.100.1
add distance=1 dst-address=172.16.60.0/24 gateway=10.100.2.10
add distance=1 dst-address=172.16.61.0/24 gateway=10.100.2.10
add distance=1 dst-address=172.16.62.0/24 gateway=10.100.2.10
add distance=1 dst-address=172.16.63.0/24 gateway=10.100.2.10
...
add local-address=172.16.86.1 name=CIBATU1 password=CIBATU123 remote-address=\
    172.16.86.2
add local-address=172.16.86.1 name=NLJAKBAR password=JAKBAR123 \
...
add default-originate=if-installed name=NL remote-address=10.100.2.10 \
    remote-as=64513
add default-originate=if-installed in-filter=SEMARANG name=SEMARANG \
    remote-address=10.100.2.18 remote-as=64518
add default-originate=if-installed in-filter=SIDOARJO name=SIDOARJO \
    remote-address=10.100.2.14 remote-as=64516
add default-originate=if-installed disabled=yes in-filter=BACKUP name=BACKUP \
    nexthop-choice=force-self remote-address=10.200.200.2 remote-as=64514
/routing filter
add action=accept chain=SEMARANG prefix=10.26.2.0/24
add action=discard chain=SEMARANG
add action=accept chain=SIDOARJO prefix=10.29.1.0/24
add action=discard chain=SIDOARJO
add action=accept chain=BACKUP prefix=0.0.0.0/0
add action=accept chain=BACKUP prefix=10.0.0.0/8 prefix-length=8-32
add action=accept chain=BACKUP prefix=172.16.0.0/16 prefix-length=16-32
add action=accept chain=BACKUP prefix=172.17.0.0/16 prefix-length=16-32
add action=accept chain=BACKUP prefix=172.18.0.0/16 prefix-length=16-32
add action=accept chain=BACKUP prefix=172.30.0.0/16 prefix-length=16-32
add action=accept chain=BACKUP prefix=182.23.0.0/16 prefix-length=16-32
add action=accept chain=BACKUP prefix=202.152.0.0/16 prefix-length=16-32
add action=accept chain=BACKUP prefix=123.231.0.0/16 prefix-length=16-32
add action=accept chain=BACKUP prefix=61.8.0.0/16 prefix-length=16-32
