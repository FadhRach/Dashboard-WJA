# nov/15/2023 04:58:40 by RouterOS 6.48.4
...
set [ find default-name=ether3 ] l2mtu=1600 name=\
    ether3-METRO-2021253682-BACKUP speed=100Mbps
...
/interface vlan
add interface=ether3-METRO-2021253682-BACKUP name=vlan-810-METRO-CERES \
    vlan-id=810
add interface=ether3-METRO-2021253682-BACKUP name=vlan-811-METRO-NIRWANA \
    vlan-id=811
add interface=ether3-METRO-2021253682-BACKUP name=vlan-813-METRO-SIDOARJO \
    vlan-id=813
add interface=ether3-METRO-2021253682-BACKUP name=vlan-814-METRO-SEMARANG \
    vlan-id=814
add interface=ether3-METRO-2021253682-BACKUP name=vlan-815-METRO-SIN vlan-id=\
    815
add interface=ether3-METRO-2021253682-BACKUP name=vlan-816-METRO-BALI \
    vlan-id=816
/interface list
...
add check-gateway=ping distance=1 dst-address=172.16.232.0/22 gateway=\
    10.12.12.33 target-scope=30
add check-gateway=ping distance=2 dst-address=172.16.232.0/22 gateway=\
    10.121.121.33
add distance=1 dst-address=172.16.236.0/24 gateway=10.100.3.1
...
add disabled=yes distance=1 dst-address=172.20.1.40/29 gateway=172.16.100.1
...
/routing bgp network
add disabled=yes network=172.20.1.40/29 synchronize=no
