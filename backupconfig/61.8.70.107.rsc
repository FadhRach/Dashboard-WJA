# jan/15/2024 04:59:52 by RouterOS 6.48
...
set [ find default-name=ether6 ] disabled=yes name=ether6-TO_AWS
set [ find default-name=ether7 ] name=ether7-FORTI_GOLDEN_UNIT
set [ find default-name=ether8 ] name=ether8-LAN_CERES
...
add name=l2tp-in3 user=nl.marketing
add name=l2tp-in4 user=ceres.fgwh
/interface vrrp
...
set default-profile=default enabled=yes ipsec-secret=nasional max-mru=2000 \
    max-mtu=1500 mrru=2000
/interface ovpn-server server
set cipher=blowfish128,aes128,aes192,aes256 enabled=yes mode=ethernet port=\
    11394 require-client-certificate=yes
...
add address=10.100.2.21/30 interface=ether7-FORTI_GOLDEN_UNIT network=\
    10.100.2.20
add address=172.20.1.41/29 interface=ether8-LAN_CERES network=172.20.1.40
/ip dns
...
add action=masquerade chain=srcnat dst-address=10.11.12.13 out-interface=\
    METRO_SINGAPORE-vlan-816 src-address=172.18.0.0/24 to-addresses=\
    10.12.12.33
add action=src-nat chain=srcnat comment="SNAT SEMARANG - SAP RISE" \
    dst-address=172.16.132.0/22 src-address=10.26.2.0/24 to-addresses=\
    172.16.100.201
add action=src-nat chain=srcnat comment="SNAT PPTP - SAP RISE" dst-address=\
    172.16.132.0/22 src-address=172.16.86.0/24 to-addresses=172.16.100.201
add action=src-nat chain=srcnat comment="SNAT L2TP - SAP RISE" dst-address=\
    172.16.132.0/22 src-address=192.168.20.0/24 to-addresses=172.16.100.201
add action=src-nat chain=srcnat comment="SNAT SIDOARJO - SAP RISE" \
...
add action=src-nat chain=srcnat dst-address=172.16.133.60 out-interface=\
    ether1-TO_FORTI src-address=172.16.0.0/22 to-addresses=172.16.100.201
add action=src-nat chain=srcnat comment="SNAT SAP-RISE-DR WIRED NL" \
    dst-address=172.16.232.0/22 src-address=172.16.48.0/20 to-addresses=\
    172.16.100.201
add action=src-nat chain=srcnat comment="SNAT SAP-RISE DR WIRELESS NL" \
    dst-address=172.16.232.0/22 src-address=172.17.240.0/20 to-addresses=\
    172.16.100.201
add action=src-nat chain=srcnat comment="SNAT SAP-RISE DR VPN NL" \
    dst-address=172.16.232.0/22 src-address=172.18.0.0/24 to-addresses=\
    172.16.100.201
add action=src-nat chain=srcnat comment="SNAT SAP-RISE DR VPN CERES" \
    dst-address=172.16.232.0/22 src-address=172.18.1.0/24 to-addresses=\
    172.16.100.201
/ip route
...
add distance=1 dst-address=172.16.236.0/24 gateway=10.100.2.1
...
add local-address=172.16.86.1 name=nl.marketing password=c3r3s!@# \
    remote-address=172.16.86.6
add local-address=172.16.86.1 name=ceres.fgwh password=c3r3s!@# \
    remote-address=172.16.86.7
/routing bgp network
...
add disabled=yes network=10.26.2.0/24 synchronize=no
...
add network=172.16.102.0/24 synchronize=no
/routing bgp peer
